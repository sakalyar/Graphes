...
R2 = []
def CycleEulerien(G, x):
    A = G[x]
    if A == set():
        return x
    #Le sommet suivant
    x2 = list(A)[0]
    #Effaçons les arcs correspondantes
    A.remove(x2)
    A2 = G[x2]
    A2.remove(x)
    G[x2] = A2
    ...
    #Appel récursif
    R2.append(CycleEulerien(G, x2))
    return x

m = 0
l = 0
#Trouvons le sommet qui nous donnera le chemin complet
for i in range(len(d)):
    if l < len(list(d[i])):
        l = len(list(d[i]))
        m = i

R2.append(CycleEulerien(d, m))
print("Le cycle eulérien: ", R2)
